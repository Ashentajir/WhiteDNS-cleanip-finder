...
**Railway's identity domain pointed at Cloudflare.** `railway.app` is fronted by Cloudflare, so it identified Cloudflare rather than Railway. Only the names a platform serves from its own address space are used now.
...
*Known limitation:* an endpoint is accepted when a CDN answers with its own signature headers, which is what lets one CDN's address space look like it serves a name hosted on another. The platform requirement narrows a scan a great deal, but on shared CDN ranges it does not fully separate one provider's edge from another's. Prefer the addresses at the top of the list — those came from the platform's own DNS answers.
